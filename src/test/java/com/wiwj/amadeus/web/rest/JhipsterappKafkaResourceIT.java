...
import com.wiwj.amadeus.config.KafkaProperties;
...
class JhipsterappKafkaResourceIT {
    ...
    void setup() {
        ...
        JhipsterappKafkaResource kafkaResource = new JhipsterappKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jhipsterapp-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/jhipsterapp-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
